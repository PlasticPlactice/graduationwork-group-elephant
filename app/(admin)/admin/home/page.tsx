        ...
        <main className="home-main">
            ...
            <div className='mt-5'>
                ...
                <div className='event-scroll-content m-auto w-3/5 py-2 px-5 shadow'>
                    {eventData.map((event) => (
                    <div key={event.id} className=' py-4 m-auto text-center event-container mb-4'>
                            
                        <div className='mx-auto flex items-center justify-between w-4/5'>
                                <p className='font-bold event-name'>{ event.title}</p>
                                <p className="event-date event-date-now">一次審査の締切：{event.date}</p>
                        </div>
                        ...
                        <div className='flex justify-between w-2/3 m-auto event-condition-status-circle'>
                            <p className='w-10'><Icon icon='material-symbols:circle' className={getCircleClassName(0, event.status)}></Icon></p>
                            <p className='w-10'><Icon icon='material-symbols:circle' className={getCircleClassName(1, event.status)}></Icon></p>
                            <p className='w-10'><Icon icon='material-symbols:circle' className={getCircleClassName(2, event.status)}></Icon></p>
                            <p className='w-10'><Icon icon='material-symbols:circle' className={getCircleClassName(3, event.status)}></Icon></p>
                        </div>
                        <div className='flex justify-center mt-2'> 
                                {/* 11,37,63,89 */}
                            <progress max={100} value={getProgressValue(event.status)} className='w-4/5'></progress>
                        </div>
                        ...
                        <button className="event-detail-btn mb-3 mt-7 p-0" onClick={handleDetail}>イベント詳細</button>
                    </div>
                    ))}
                </div>
            </div>
